  ...
  **Warning**: A statement belonging to a database that has been closed cannot
  ...
  Database.prototype.handleError = function(returnCode) {
    ...
    if (returnCode === SQLite.OK) {
      ...
      try {
        console.log("BORA 1", returnCode);
        errmsg = sqlite3_errmsg(this.db);
        var errcodeext = sqlite3_extended_errcode(this.db);
        console.log("BORA 1", errcodeext, errmsg);
      } catch (e) {
        console.log("ROLLING DEEP", e);
      }
      errmsg = sqlite3_errmsg(this.db);
...
sqlite3_extended_errcode = Module['cwrap']('sqlite3_extended_errcode', 'number', ['number']);
...
if (typeof importScripts === 'function') {
  ...
  self.onmessage = function(event) {
    return sqlModuleReady.then(function() {
      var buff, callback, data, done, err, result, url;
      data = event['data'];
